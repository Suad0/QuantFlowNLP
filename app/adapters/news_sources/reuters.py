"""Reuters RSS feed adapter.

This module implements the news source adapter for Reuters RSS feeds,
...
class ReutersAdapter(NewsSourceAdapter):
    """Adapter for fetching articles from Reuters RSS feeds.
    
    Reuters provides RSS feeds for various news categories including finance
    and business news.
    """
    ...
    async def fetch(self) -> list[RawArticle]:
        """Fetch articles from Reuters RSS feed.
        ...
        try:
            ...
            raise NewsIngestionError(
                f"Failed to fetch Reuters RSS feed: {e}"
            ) from e
        except ET.ParseError as e:
            raise NewsIngestionError(
                f"Failed to parse Reuters RSS feed: {e}"
            ) from e
    ...
    def _parse_rss(self, rss_content: str) -> list[RawArticle]:
        ...
        try:
            ...
            raise NewsIngestionError(
                f"Error parsing Reuters RSS items: {e}"
            ) from e
    ...
    def _parse_item(self, item: ET.Element) -> Optional[RawArticle]:
        ...
        try:
            ...
            # Reuters may use content:encoded for full content
