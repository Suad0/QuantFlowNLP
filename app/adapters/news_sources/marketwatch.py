"""MarketWatch RSS feed adapter.

This module implements the news source adapter for MarketWatch RSS feeds,
...
class MarketWatchAdapter(NewsSourceAdapter):
    """Adapter for fetching articles from MarketWatch RSS feeds.
    
    MarketWatch provides RSS feeds for various financial news categories.
    This adapter fetches from the top stories feed.
    """
    
    RSS_URL = "https://www.marketwatch.com/rss/topstories"
    
    def __init__(self, timeout: int = 30, max_articles: int = 100) -> None:
        """Initialize MarketWatch adapter.
    ...
    async def fetch(self) -> list[RawArticle]:
        """Fetch articles from MarketWatch RSS feed.
        ...
        try:
            ...
            raise NewsIngestionError(
                f"Failed to fetch MarketWatch RSS feed: {e}"
            ) from e
        except ET.ParseError as e:
            raise NewsIngestionError(
                f"Failed to parse MarketWatch RSS feed: {e}"
            ) from e
    ...
    def _parse_rss(self, rss_content: str) -> list[RawArticle]:
        ...
        try:
            ...
            raise NewsIngestionError(
                f"Error parsing MarketWatch RSS items: {e}"
            ) from e
    ...
    def _parse_item(self, item: ET.Element) -> Optional[RawArticle]:
        ...
        try:
            ...
            # MarketWatch may use content:encoded for full content
